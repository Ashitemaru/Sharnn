...
class RNN {
    ...
    RNN(int _input_w, int _hidden_w, int _bias, int _hidden = 0, RNNType type = RNNType::Naive)
        : hidden(_hidden), init_hidden(_hidden) {
        if (type == RNNType::Naive) cell = new NaiveRNNCell(_input_w, _hidden_w, _bias);
        else if (type == RNNType::LSTM) cell = new LSTMCell();
        else assert(false);
    }
